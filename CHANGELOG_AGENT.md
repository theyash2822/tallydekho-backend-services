...
## 2026-07-06 R6 — Tally Master Fetch (country + state) via new /api/tally/masters routes

### Context
After R2 mailing fix, R3 website + VAT fix, and R4 mobile bank/multi-country UI, user reported: state dropdown in Party Form still showed INDIAN_STATES even when a non-India country was picked (e.g. South Africa → Punjab/Kerala wrongly listed). User provided 2 TDL Collection XMLs from Tally documentation and asked: can we fetch Tally's own country + state master lists directly? Answer: yes, via new backend routes that reuse the existing desktop WebSocket pipeline.

### Added
- **NEW `src/routes/tally-read.js`** — auth-guarded route family for Tally READ operations. Contains 2 endpoints:
  - `GET /api/tally/masters/countries?companyGuid=...` — cache-first, live-fetch fallback, empty on failure.
  - `GET /api/tally/masters/states?companyGuid=...&country=...` — same pattern, optional country filter (case-insensitive).
- **Helpers inside route file:**
  - `fetchFromTallyViaDesktop(companyGuid, userId, xmlBody)` — emits `tally:read` WebSocket event to paired desktop, 5s timeout, returns raw XML.
  - `parseCountriesXml(xml)` — regex-based, handles both `<COUNTRY NAME="...">` attribute and nested `<NAME>` shapes. Deduplicated + sorted.
  - `parseStatesXml(xml)` — extracts `{name, country, gstStateCode}` tuples, dedup by name+country, sorted by country then name.
  - `isMetaValue(s)` — filters out '♦ Not Applicable', 'Any Country', 'Any State', empty strings, sentinels.
  - DB helpers: `readCountriesCache`, `readStatesCache`, `writeCountriesCache`, `writeStatesCache`.
- **XML constants:** `XML_COUNTRIES` + `XML_STATES` — hardcoded TDL Collection payloads (user-provided, verified format).

### Schema (idempotent CREATE IF NOT EXISTS)
- `tally_country_master (company_guid, name, fetched_at)` — PK (company_guid, name)
- `tally_state_master (company_guid, name, country, gst_state_code, fetched_at)` — PK (company_guid, name, country) + index on (company_guid, country)

### Wiring (`src/server.js`)
- Import `tallyReadRoutes, setTallyReadSocket` from `./routes/tally-read.js`.
- `app.use('/api/tally', tallyReadRoutes)` — placed BEFORE `/api` catch-all.
- `setTallyReadSocket(socketService)` — adjacent to existing `setTallyWriteSocket`.

### Flow (typical happy path)
1. Mobile Party Form mounts → calls `GET /api/tally/masters/countries + /states` (parallel).
2. Backend hits `tally_country_master WHERE company_guid=?`. If rows → return instantly (cache hit).
3. Cache miss → backend emits `tally:read` event to desktop via existing WebSocket with TDL Collection XML.
4. Desktop calls `fetchFromTally()` → Tally :9000 responds with collection XML (~200-500ms).
5. Desktop returns raw XML to backend via WebSocket callback.
6. Backend parses via regex, filters sentinels, saves to DB, returns JSON to mobile.
7. All subsequent calls hit cache (instant).

### Failure paths (all graceful, never break form)
- No paired desktop → return empty data, mobile falls back to hardcoded lists.
- Desktop offline (WebSocket disconnected) → same as above.
- Tally not running / not reachable → same as above.
- Live fetch timeout (5s) → same as above.
- Parse error (XML malformed) → same as above.
- Every path returns `{ status: true, data: [] }` with a `note` field — mobile treats empty as "use hardcoded fallback".

### Not changed
- `tally-write.js` — R1-R5 XML wins all preserved (LEDMAILINGDETAILS, COUNTRY variants, STATE variants, VAT TIN/CST tags, LEDGERWEBSITE etc.).
- All existing routes untouched.
- Existing WebSocket `tally:write` handler on desktop — untouched.
- Existing `forwardToTally` helper on backend — untouched.
- Mobile write payloads (`bankDetails` removal, `vatDetails` structure) — unchanged.

### QA
🟢 GREEN — LITE subagent (14 checks all passed): backend syntax (3 files), route structure + helpers + XML constants, schema idempotency, server.js wiring order, desktop syntax (2 files), fetchFromTally + tally:read handler parity with tally:write, mobile TypeScript 0 errors, api.ts new functions, PartyForm.tsx dropdown wiring, parent callers (ledger/create + create-invoice) pass companyGuid, endpoint auth-guarded (401 not 404), R1-R5 XML wins regression clean.

### Files
- `src/routes/tally-read.js` (NEW)
- `src/db/schema.js` (modified)
- `src/server.js` (modified)

### Commit
`f4e06cd` on `main`

### 🔴 Pending user verification (device test after desktop restart)
1. User must `git pull` in desktop repo + restart desktop app (to activate new `tally:read` handler).
2. Open Party Form on mobile → background fetch fires → DB populates (first ever open per company).
3. Verify: dropdown shows Tally's actual country list (may differ from hardcoded 25 depending on Tally installation).
4. Pick South Africa → state dropdown should now show SA provinces (Gauteng, Western Cape, etc.), NOT Indian states.
5. Pick India → state dropdown shows Indian states (either from Tally or hardcoded fallback — both should match).
6. Check backend DB: `SELECT COUNT(*) FROM tally_country_master WHERE company_guid = 'a6ea3d30-e7c3-4412-bb7f-6681a130bcb9';` → should be non-zero after first fetch.
