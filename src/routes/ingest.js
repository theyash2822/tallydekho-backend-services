...
} from '../services/deviceCompanyResolution.js';
...
router.post('/desktop/init-sync', requireDeviceCredential, async (req, res) => {
  ...
  try {
    ...
    // Phase 3E: same Tally GUID may exist in another workspace as a separate company.
    // assertCompanyGuidAvailableForWorkspace only checks same-workspace reuse/create.
    for (const g of incomingGuids) {
      try {
        await assertCompanyGuidAvailableForWorkspace(workspaceId, g);
      } catch (err) {
    ...
    if (companies) {
      for (const c of companies) {
        try {
          ...
          // Phase 3E: conflict target is (workspace_id, guid) — never steal another tenant's row.
          const companyAddress = Array.isArray(c.address)
            ? c.address.map((l) => String(l || '').trim()).filter(Boolean).join(', ')
            : (c.address || null);
          const { rows: savedCo } = await query(`
            INSERT INTO companies (guid, device_id, workspace_id, name, formal_name, gstin, fy_start, fy_end, synced_at, is_active, gst_taxpayer_type,
    ...
    if (workspaceId && companies?.length) {
      // CID-F001: do NOT force reassign workspace_id for arbitrary guids (ownership steal removed).
      // Companies created/updated above already bind workspace_id on insert; same-workspace updates only.
...
router.post('/ingest/chunk', requireDeviceCredential, async (req, res) => {
  ...
  try {
    ...
    // Phase 4: workspace binding is authority — do not require devices.user_id
    if (!device.workspace_id && !device.paired) {
      return res.status(403).json({ status: false, message: 'Device not paired. Complete pairing first.' });
    }
    ...
    if (resolvedCompanyId) {
      try {
        ...
        if (staleAnchors.length > 0) {
          ...
          await query(
            `DELETE FROM ledger_fy_balances WHERE company_id = $1 AND financial_year = ANY($2::text[])`,
            [resolvedCompanyId, staleYears]
          );
      ...
      if (resolvedCompanyId) {
        const { rowCount } = await query(`
          ...
          WHERE v.guid = tt.voucher_guid
            AND v.company_id = tt.company_id
            AND tt.company_id = $1
